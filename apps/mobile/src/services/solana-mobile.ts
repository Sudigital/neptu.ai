...
// Base64 decode without Node Buffer (not available in RN)
...
export async function connectWallet(): Promise<{
  ...
  const result = await transact(async (wallet: Web3MobileWallet) => {
    ...
    // MWA returns address as base64-encoded bytes — decode to PublicKey
    const addressBytes = base64ToBytes(auth.accounts[0].address);
...
export async function signMessage(
  ...
  const result = await transact(async (wallet: Web3MobileWallet) => {
    ...
    return bytesToBase64(new Uint8Array(signatures[0]));
  });
  return result;
}
...
export async function signAndSendTransaction(
  ...
  const result = await transact(async (wallet: Web3MobileWallet) => {
    ...
    return signatures[0] ? bytesToBase64(new Uint8Array(signatures[0])) : "";
  });
  return result;
}
