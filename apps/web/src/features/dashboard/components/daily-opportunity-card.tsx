    ...
    <div className="rounded-xl border bg-card p-4 shadow-sm">
      <div className="mb-3 flex items-center gap-2">
        ...
        <span className="text-sm font-semibold">
          {(() => {
            if (isToday(selectedDate)) return t("dashboard.todaysOpportunity");
            if (isPast(selectedDate))
              return `${format(selectedDate, "MMM d")} ${t("dashboard.pastReading")}`;
            return `${format(selectedDate, "MMM d")} ${t("dashboard.futurePrediction")}`;
          })()}
        </span>
      </div>
